<?php
/**
 * Class ImportDeliveryOrder
 * @property \App\ImportDeliveryOrder $model
 */

namespace App\Http\Controllers\Admin;

use App\ImportDeliveryOrder;
use Breadcrumb;
...
class ImportDeliveryOrderController extends Controller
{
    ...
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'import_delivery_orders.' . $this->id_key])->merge(request()->query())->toArray();

        /** -------- Query */
        $select = "import_delivery_orders.id
, import_delivery_orders.file
, import_delivery_orders.oem_id
-- , import_delivery_orders.oem_id
, import_delivery_orders.status

    ";
        $SQL = $this->model->select(\DB::raw($select));

        //$SQL = $SQL->leftJoin('oems', 'oems.id', '=', 'import_delivery_orders.oem_id');
        /** -------- WHERE */
        $where = $this->where;
        //$select = $SQL;
