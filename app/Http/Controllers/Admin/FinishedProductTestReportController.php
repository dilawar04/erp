<?php
/**
 * Class FinishedProductTestReport
 * @property \App\FinishedProductTestReport $model
 */

namespace App\Http\Controllers\Admin;

use App\FinishedProductTestReport;
use Breadcrumb;
...
class FinishedProductTestReportController extends Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method FinishedProductTestReport index | Grid | listing
     * *****************************************************************************************************************
     */
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'finished_product_test_reports.' . $this->id_key])->merge(request()->query())->toArray();

        /** -------- Query */
        $select = "finished_product_test_reports.id
, finished_product_test_reports.date
, finished_product_test_reports.schedule as upload_document 
, finished_product_test_reports.status
, finished_product_test_reports.created_at

    ";
        $SQL = $this->model->select(\DB::raw($select));

        //$SQL = $SQL->leftJoin('oems', 'oems.id', '=', 'finished_product_test_reports.oem_id');
        /** -------- WHERE */
        $where = $this->where;
        //$select = $SQL;
    ...
    /**
     * *****************************************************************************************************************
     * @method finished_product_test_reports form
     * *****************************************************************************************************************
     */
    ...
    /**
     * *****************************************************************************************************************
     * @method finished_product_test_reports store | Insert & Update
     * *****************************************************************************************************************
     */
