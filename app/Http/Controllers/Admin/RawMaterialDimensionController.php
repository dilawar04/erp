<?php
/**
 * Class RawMaterialDimension
 * @property \App\RawMaterialDimension $model
 */

namespace App\Http\Controllers\Admin;

use App\RawMaterialDimension;
use Breadcrumb;
...
class RawMaterialDimensionController extends Controller
{
    ...
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'raw_material_dimensions.' . $this->id_key])->merge(request()->query())->toArray();

        /** -------- Query */
        $select = "raw_material_dimensions.id
, raw_material_dimensions.item
, raw_material_dimensions.value
...
, raw_material_dimensions.created_at
    ...
    ";
        ...
        //$SQL = $SQL->leftJoin('users', 'users.id', '=', 'raw_material_dimensions.user_id');
                /** -------- WHERE */
        $where = $this->where;
        //$select = $SQL;
