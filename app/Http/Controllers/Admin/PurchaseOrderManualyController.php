<?php
/**
 * Class PurchaseOrderManualy
 * @property \App\PurchaseOrderManualy $model
 */

namespace App\Http\Controllers\Admin;

use App\PurchaseOrderManualy;
use Breadcrumb;
...
class PurchaseOrderManualyController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->table = $this->model->getTable('suppliers');
                $this->table = $this->model->getTable('raw_material_profiles');
        $this->id_key = $this->model->getKeyName(suppliers);
        $this->id_key = $this->model->getKeyName(raw_material_profiles);
    ...
    /**
     * *****************************************************************************************************************
     * @method purchase_order_manuallies index | Grid | listing
     * *****************************************************************************************************************
     */
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'purchase_order_manualies.' . $this->id_key])->merge(request()->query())->toArray();

        /** -------- Query */
        $select = "purchase_order_manualies.id
, IFNULL(parent_purchase_order_manualies.title, 'Main') AS parent
, purchase_order_manualies . title
, purchase_order_manualies.date
, purchase_order_manualies.material
, purchase_order_manualies.unit
, purchase_order_manualies.quantity_per_pack
, purchase_order_manualies.moq
, purchase_order_manualies.rate
, purchase_order_manualies.order_quantity
, purchase_order_manualies.total_amount
, purchase_order_manualies.lead_time
, purchase_order_manualies.expected_delivery_date
, purchase_order_manualies.grand_total
, purchase_order_manualies.status
, purchase_order_manualies.created_at
    ";
        $SQL = $this->model->select(\DB::raw($select));
        
                $SQL = $SQL->leftJoin('purchase_order_manualies as parent_purchase_order_manualies', 'parent_purchase_order_manualies.id', '=', 'purchase_order_manualies.parent_id');
        //$SQL = $SQL->leftJoin('users', 'users.id', '=', 'purchase_order_manuallies.user_id');
        /** -------- WHERE */
        $where = $this->where;
        //$select = $SQL;
    ...
    /**
     * *****************************************************************************************************************
     * @method purchase_order_manuallies form
     * *****************************************************************************************************************
     */
    ...
    /**
     * *****************************************************************************************************************
     * @method purchase_order_manuallies store | Insert & Update
     * *****************************************************************************************************************
     */
    ...
    /**
     * *****************************************************************************************************************
     * @method workstations view | Record
     * *****************************************************************************************************************
     */
    ...
    /**
     * *****************************************************************************************************************
     * @method workstations AJAX actions
     * *****************************************************************************************************************
     */
    function ajax()
    {
