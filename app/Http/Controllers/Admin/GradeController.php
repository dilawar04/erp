...
class GradeController extends Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method grades index | Grid | listing
     * *****************************************************************************************************************
     */
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'grades.' . $this->id_key])->merge(request()->query())->toArray();

        /** -------- Query */
        $select = "grades.id
, grades.name
, grades.salary_range_from
, grades.salary_range_to
, grades.status
";
        $SQL = $this->model->select(\DB::raw($select));

        //$SQL = $SQL->leftJoin('allowance_benefits', 'allowance_benefits.id', '=', 'grades.allowance_id');
