<?php
/**
 * Class DeliveryOrderManually
 * @property \App\DeliveryOrderManually $model
 */

namespace App\Http\Controllers\Admin;

use App\DeliveryOrderManually;
use Breadcrumb;
...
class DeliveryOrderManuallyController extends Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method delivery_order_manuallies index | Grid | listing
     * *****************************************************************************************************************
     */
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'delivery_order_manuallies.' . $this->id_key])->merge(request()->query())->toArray();

        /** -------- Query */
        $select = "delivery_order_manuallies.id
...
, delivery_order_manuallies.created_at
    ";
        $SQL = $this->model->select(\DB::raw($select));
        
                $SQL = $SQL->leftJoin('oems', 'oems.id', '=', 'delivery_order_manuallies.oem_id');
