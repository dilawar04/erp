 ...
 * @property \App\FinishedProductStock $model
 */

namespace App\Http\Controllers\Admin;

use App\FinishedProductStock;
use Breadcrumb;
...
class FinishedProductStockController extends Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method finished_product_stocks index | Grid | listing
     * *****************************************************************************************************************
     */
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'finished_product_stocks.' . $this->id_key])->merge(request()->query())->toArray();

        /** -------- Query */
        $select = "finished_product_stocks.id
, finished_product_stocks.opening_quantity
, finished_product_stocks.stock_rate
, finished_product_stocks.stock_value
, finished_product_stocks.opening_date
, finished_product_stocks.unit_id
, finished_product_stocks.status
, finished_product_stocks.created_at

    ";
        $SQL = $this->model->select(\DB::raw($select));

        //$SQL = $SQL->leftJoin('users', 'users.id', '=', ', finished_product_stocks.status.user_id');
                /** -------- WHERE */
        $where = $this->where;
        //$select = $SQL;
    ...
    /**
     * *****************************************************************************************************************
     * @method , finished_product_stocks.status store | Insert & Update
     * *****************************************************************************************************************
     */
