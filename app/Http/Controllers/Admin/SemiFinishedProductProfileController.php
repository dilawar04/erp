...
/**
 * Class Oem
 * @property \App\Oem $model
 */

namespace App\Http\Controllers\Admin;

use App\SemiFinishedProductProfile;
use Breadcrumb;
...
class SemiFinishedProductProfileController extends Controller
{
    ...
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'semi_finished_product_profiles.' . $this->id_key])->merge(request()->query())->toArray();

        /** -------- Query */
        $select = "semi_finished_product_profiles.id
, semi_finished_product_profiles.product_code
, semi_finished_product_profiles.product_name
, semi_finished_product_profiles.parent_product
, semi_finished_product_profiles.process_location
, semi_finished_product_profiles.workstation_id
, semi_finished_product_profiles.line_of_list
, semi_finished_product_profiles.daily_production_capacity
, semi_finished_product_profiles.safety_stock
, semi_finished_product_profiles.mode_of_storage
, semi_finished_product_profiles.quantity_pack
, semi_finished_product_profiles.wip_bin
, semi_finished_product_profiles.store
, semi_finished_product_profiles.storage_level_1
, semi_finished_product_profiles.storage_level_2
, semi_finished_product_profiles.exclusive_location
, semi_finished_product_profiles.status
, semi_finished_product_profiles.created_at
    ";
        $SQL = $this->model->select(\DB::raw($select));

        //$SQL = $SQL->leftJoin('users', 'users.id', '=', 'oems.user_id');
                /** -------- WHERE */
        $where = $this->where;
        //$select = $SQL;
    ...
    /**
     * *****************************************************************************************************************
     * @method oems store | Insert & Update
     * *****************************************************************************************************************
     */
    public function store()
    {
        ...
        $validator_rules = [
                ...
                'process_location' => "required",
                'line_of_list' => "required",
                'daily_production_capacity' => "required",
                'safety_stock' => "required",
                'mode_of_storage' => "required",
                'quantity_pack' => "required",
                'store' => "required",
                'exclusive_location' => "required",
