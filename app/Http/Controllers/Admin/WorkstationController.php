...
/**
 * Class WorkStation
 * @property \App\WorkStation $model
 */
...
class WorkStationController extends Controller
{
    ...
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'work_stations.' . $this->id_key])->merge(request()->query())->toArray();

        /** -------- Query */
        $select = "work_stations.id
, work_stations.code
, work_stations.name
, work_stations.status
, work_stations.created_at

    ";
        $SQL = $this->model->select(\DB::raw($select));

        //$SQL = $SQL->leftJoin('users', 'users.id', '=', 'work_stations.user_id');
                /** -------- WHERE */
        $where = $this->where;
        //$select = $SQL;
    ...
    /**
     * *****************************************************************************************************************
     * @method work_stations store | Insert & Update
     * *****************************************************************************************************************
     */
    ...
    /**
     * *****************************************************************************************************************
     * @method work_stations view | Record
     * *****************************************************************************************************************
     */
    ...
    /**
     * *****************************************************************************************************************
     * @method work_stations AJAX actions
     * *****************************************************************************************************************
     */
    function ajax()
    {
