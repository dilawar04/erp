<?php
/**
 * Class FinishedProductImage
 * @property \App\FinishedProductImage $model
 */

namespace App\Http\Controllers\Admin;

use App\FinishedProductImage;
use Breadcrumb;
...
class FinishedProductImageController extends Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method FinishedProductImage index | Grid | listing
     * *****************************************************************************************************************
     */
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'finished_product_images.' . $this->id_key])->merge(request()->query())->toArray();

        /** -------- Query */
        $select = "finished_product_images.id
, finished_product_images.date
, finished_product_images.schedule as upload_document 
, finished_product_images.status
, finished_product_images.created_at

    ";
        $SQL = $this->model->select(\DB::raw($select));

        //$SQL = $SQL->leftJoin('oems', 'oems.id', '=', 'finished_product_images.oem_id');
        /** -------- WHERE */
        $where = $this->where;
        //$select = $SQL;
    ...
    /**
     * *****************************************************************************************************************
     * @method finished_product_images form
     * *****************************************************************************************************************
     */
    ...
    /**
     * *****************************************************************************************************************
     * @method finished_product_images store | Insert & Update
     * *****************************************************************************************************************
     */
