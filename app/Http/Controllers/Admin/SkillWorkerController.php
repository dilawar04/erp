...
/**
 * Class SkillWorker
 * @property \App\SkillWorker $model
 */
...
class SkillWorkerController extends Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method skills index | Grid | listing
     * *****************************************************************************************************************
     */
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'skill_workers.' . $this->id_key])->merge(request()->query())->toArray();
...
, skill_workers.status
    ...
    /**
     * *****************************************************************************************************************
     * @method skills store | Insert & Update
     * *****************************************************************************************************************
     */
    public function store()
    {
        ...
        if ($id > 0) {
            ...
            $operationIdArray = json_decode($data['data']['operation_id'], true);
            $operationWeightageArray = json_decode($data['data']['operation_weightage'], true);

            $row->name = $nameArray[0];
            $row->workstation_id = $workstationIdArray[0];
            $row->operation_id = $operationIdArray[0];
            $row->operation_weightage = $operationWeightageArray[0];
            $row->save();
            ...
            $row = new SkillWorker(); // Replace YourModel with your model class name
            foreach ($nameArray as $key => $name) {
                $row->create([
                    'name' => $name,
                    'workstation_id' => $workstationIdArray[$key],
                    'operation_id' => $operationIdArray[$key],
                    'operation_weightage' => $operationWeightageArray[$key],
                ]);
            }
        }
