<?php
/**
 * Class SemiFinishedProductDimension
 * @property \App\SemiFinishedProductDimension $model
 */

namespace App\Http\Controllers\Admin;

use App\SemiFinishedProductDimension;
use Breadcrumb;
...
class SemiFinishedProductDimensionController extends Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method raw_material_dimensions index | Grid | listing
     * *****************************************************************************************************************
     */
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'semi_finished_product_dimensions.' . $this->id_key])->merge(request()->query())->toArray();

        /** -------- Query */
        $select = "semi_finished_product_dimensions.id
, semi_finished_product_dimensions.item
, semi_finished_product_dimensions.value
, semi_finished_product_dimensions.tolerance
, semi_finished_product_dimensions.unit_id
, semi_finished_product_dimensions.method
, semi_finished_product_dimensions.inspection_id
, semi_finished_product_dimensions.sampling_criteria
, semi_finished_product_dimensions.sample_size
, semi_finished_product_dimensions.remarks
, semi_finished_product_dimensions.status
, semi_finished_product_dimensions.created_at

    ";
           $SQL = $this->model->select(\DB::raw($select));

        //$SQL = $SQL->leftJoin('users', 'users.id', '=', 'semi_finished_product_dimensions.user_id');
                /** -------- WHERE */
        $where = $this->where;
        //$select = $SQL;
    ...
    /**
     * *****************************************************************************************************************
     * @method raw_material_dimensions form
     * *****************************************************************************************************************
     */
    ...
    /**
     * *****************************************************************************************************************
     * @method raw_material_dimensions store | Insert & Update
     * *****************************************************************************************************************
     */
    public function store()
    {
        ...
        if ($status = $row->save()) {
            ...
            /** -------- Sub Department */
            $_ids = request('ids');
            $sub_table = $this->table;
            $sub_data['parent_id'] = $id;
            foreach (request('sub_departments') as $k => $value){
                $sub_data['title'] = $value;
                $_where  = "";
                if(in_array($k, $_ids)){
                    $_where = "id='{$k}'";
                }
                save($sub_table, $sub_data, $_where);
            }
            if(count($_ids) > 0){
                \App\Department::whereNotIn('id',$_ids)->where('parent_id', $id)->delete();
            }
            /** -------- End */
        } else {
    ...
    /**
     * *****************************************************************************************************************
     * @method raw_material_dimensions delete
     * *****************************************************************************************************************
     */
    ...
    /**
     * *****************************************************************************************************************
     * @method departments view | Record
     * *****************************************************************************************************************
     */
    ...
    /**
     * *****************************************************************************************************************
     * @method raw_material_dimensions AJAX actions
     * *****************************************************************************************************************
     */
    function ajax()
    {
    ...
    /**
     * *****************************************************************************************************************
     * @method raw_material_dimensions duplicate
     * *****************************************************************************************************************
     */
    ...
    /**
     * *****************************************************************************************************************
     * @method raw_material_dimensions export
     * @type csv & xml
     * *****************************************************************************************************************
     */
