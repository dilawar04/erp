...
/**
 * Class ModeStorage
 * @property \App\ModeStorage $model
 */
...
class ModeStorageController extends Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method ModeStorages index | Grid | listing
     * *****************************************************************************************************************
     */
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'mode_storages.' . $this->id_key])->merge(request()->query())->toArray();

        /** -------- Query */
        $select = "mode_storages.id
, IFNULL(parent_mode_storages.title, 'Main') AS parent
, mode_storages.title
, mode_storages.status

    ";
        $SQL = $this->model->select(\DB::raw($select));

        $SQL = $SQL->leftJoin('mode_storages as parent_mode_storages', 'parent_mode_storages.id', '=', 'mode_storages.parent_id');
    ...
    /**
     * *****************************************************************************************************************
     * @method ModeStorages store | Insert & Update
     * *****************************************************************************************************************
     */
    public function store()
    {
        ...
        if ($status = $row->save()) {
            ...
            foreach (request('sub_mode_storages') as $k => $value){
            ...
            if(count($_ids) > 0){
                \App\ModeStorage::whereNotIn('id',$_ids)->where('parent_id', $id)->delete();
            }
            /** -------- End */
        } else {
    ...
    /**
     * *****************************************************************************************************************
     * @method ModStorages view | Record
     * *****************************************************************************************************************
     */
    ...
    /**
     * *****************************************************************************************************************
     * @method ModeStorages AJAX actions
     * *****************************************************************************************************************
     */
    function ajax()
    {
