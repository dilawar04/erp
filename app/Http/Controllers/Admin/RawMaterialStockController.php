 ...
 * @property \App\RawMaterialStock $model
 */
...
class RawMaterialStockController extends Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method raw_material_stocks index | Grid | listing
     * *****************************************************************************************************************
     */
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'raw_material_stocks.' . $this->id_key])->merge(request()->query())->toArray();

        /** -------- Query */
        $select = "raw_material_stocks.id
, raw_material_stocks.opening_quantity
, raw_material_stocks.stock_rate
, raw_material_stocks.stock_value
, raw_material_stocks.opening_date
, raw_material_stocks.unit_id
, raw_material_stocks.status
, raw_material_stocks.created_at

    ";
        $SQL = $this->model->select(\DB::raw($select));

        //$SQL = $SQL->leftJoin('users', 'users.id', '=', ', raw_material_stocks.status.user_id');
                /** -------- WHERE */
        $where = $this->where;
        //$select = $SQL;
    ...
    /**
     * *****************************************************************************************************************
     * @method , raw_material_stocks.status form
     * *****************************************************************************************************************
     */
    ...
    /**
     * *****************************************************************************************************************
     * @method , raw_material_stocks.status store | Insert & Update
     * *****************************************************************************************************************
     */
