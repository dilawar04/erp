...
/**
 * Class PurchaseKpi
 * @property \App\PurchaseKpi $model
 */
...
class PurchaseKpiController extends Controller
{
 ...
 /**
    ...
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'purchase_kpis.' . $this->id_key])->merge(request()->query())->toArray();

        /** -------- Query */
        $select = "purchase_kpis.id
, purchase_kpis.purchase_proposal_approval_lead_time	
, purchase_kpis.purchase_order_approval_lead_time
, purchase_kpis.purchase_order_issue_lead_time
, purchase_kpis.status
, purchase_kpis.created_at
 ...
 ";
    ...
    public function store()
    {
       $id = request()->input($this->id_key);
        ...
        /** -------- Validation */
        $validator_rules = [
                        'purchase_order_issue_lead_time' => "required",
        ...
        /** -------- Upload Files */
        $files = upload_files(['schedule', 'icon', 'file'], "assets/front/{$this->table}/");
        if (count($files) > 0) {
