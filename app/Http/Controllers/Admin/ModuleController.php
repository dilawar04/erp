...
class ModuleController extends Controller
{
    ...
    {
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'modules.' . $this->id_key])->merge(request()->query())->toArray();
        ...
        if (request()->ajax()) {
            ...
            return view('admin.modules.grid', compact('paginate_OBJ', 'query'));
        }
    }
    ...
    public function store()
    {
        ...
        if ($row->save()) {
            ...
            \Cache::forget("mod_actions_{$row->module}");
            /*
             * ---------------------------------------------------------------------------------------------------------
             * Add Module
             * ---------------------------------------------------------------------------------------------------------
    ...
    public function delete_with_files()
    {
        ...
        $rows = \App\Module::whereIn('id', $ids)->get();
        foreach ($rows as $row) {
            $m_name = Str::studly(Str::singular($row->module));
            $files = [
                base_path("app/Http/Controllers/Admin/{$m_name}Controller.php"),
                base_path("app/{$m_name}.php"),
                base_path("database/factories/{$m_name}Factory.php"),
                base_path("database/migrations/create_{$row->module}_table.php"),
                base_path("app/Imports/{$m_name}Import.php"),
            ];
            File::delete($files);

            $dirs = [
                "resources/views/admin/{$row->module}",
                "public/assets/front/{$row->module}",
            ];
            foreach ($dirs as $dir) {
                File::deleteDirectory(base_path($dir));
            }
            $unlink = ['icon' => asset_dir('media/icons/', true), 'image' => asset_dir('media/icons/', true)];
            delete_rows($this->table, "{$this->id_key} ='{$row->id}'", true, $unlink);
            delete_rows('user_type_module_rel', "module_id='{$row->id}'");

            \Schema::dropIfExists($row->module);
        }
