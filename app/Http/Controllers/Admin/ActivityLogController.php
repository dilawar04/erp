...
class ActivityLogController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $SUPER_LEVEL = env('SUPER_LEVEL', 1000001);
        if(\Auth::user()->usertype()->level < $SUPER_LEVEL){
            $sub_query = "select users.id
                from users
                inner join user_types ON(user_types.id = users.user_type_id)
                where user_types.level >= {$SUPER_LEVEL}";
            $this->where .= " AND {$this->table}.user_id NOT IN({$sub_query})";
        }
...
";
        ...
        if (request()->ajax() || request()->is('api/*')) {
            ...
            return view('admin.activity_logs.grid', compact('paginate_OBJ', 'query'));
        }
    }
    ...
    public function form()
    {
        ...
        /** -------- Response */
        \View::share('_this', (object)get_object_vars($this));
        return view('admin.activity_log.form', compact('row'));
    }
    ...
    public function view()
    {
        ...
        if (request()->ajax() || request()->is('api/*')) {
            ...
            return view('admin.activity_logs.view', compact('row', 'config'));
        } else {
