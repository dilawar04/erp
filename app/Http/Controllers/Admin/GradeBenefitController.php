...
/**
 * Class GradeBenefit
 * @property \App\GradeBenefit $model
 */

namespace App\Http\Controllers\Admin;

use App\GradeBenefit;
use Breadcrumb;
...
class GradeBenefitController extends Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method GradeBenefit index | Grid | listing
     * *****************************************************************************************************************
     */
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'grade_benefits.' . $this->id_key])->merge(request()->query())->toArray();

        /** -------- Query */
        $select = "grade_benefits.id
, grade_benefits.grade_id
, grade_benefits.employee_contribution
, grade_benefits.employers_contribution
, grade_benefits.total
, grade_benefits.leaves_id
, grade_benefits.status
";
        $SQL = $this->model->select(\DB::raw($select));

        $SQL = $SQL->leftJoin('grades', 'grades.id', '=', 'grade_benefits.grade_id'); 
                //$SQL = $SQL->leftJoin('leaves', 'leaves.id', '=', 'grades.leaves_id');
                /** -------- WHERE */
        $where = $this->where;
        //$select = $SQL;
    ...
    /**
     * *****************************************************************************************************************
     * @method grades store | Insert & Update
     * *****************************************************************************************************************
     */
