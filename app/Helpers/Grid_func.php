...
function grid_img($value, $width = null, $height = null, $attr = []){
    $file_path = public_path(str_replace(asset(''), '', $value));
    ...
    return '<a data-fancybox="gallery" href="'.$value.'">
                <img src="'._img($icon_url, $width, $height).'" class="img-fluid img_center -kt-img-rounded shadow-sm" data-skin="dark" data-toggle="kt-tooltip" title="click to zoom">
            </a>';
}
...
function _date_format($value, $row, $field_name, $grid)
{
    return \Carbon\Carbon::make($value)->format('Y-m-d H:i');
}
