@php
    $status_column_data = DB_enumValues('delivery_order_manuallies', 'status');
...
@section('content')
    ...
    <form action="{{ admin_url('', true) }}" method="get" enctype="multipart/form-data" id="delivery_order_manuallies-form">
        @csrf
        @include('admin.layouts.inc.stickybar', compact('form_buttons'))
        <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
            <div class="row">
                <div class="col-lg-12">
                    <div class="kt-portlet" data-ktportlet="true" id="kt_portlet_delivery_order_manuallies">
                        ...
                        <div class="kt-portlet__body kt-padding-0">
                            @php
                                $params = [
                                    ...
                                    'href' => admin_url('oems/form/?oem_id={_id}'),
                                    //'href' => admin_url('oems?wid={_id}'),
                                    //'href' => '#modal',
                                    'attr' => 'target="_blank"',
                                    'icon_cls' => 'la la-plus',
                                ];
                                ...
                                $params = [
                                    'title' => 'Logs',
                                    'href' => admin_url('oem_logs/{_id}', 1),
                                    'icon_cls' => 'la la-crosshairs',
                                ];
                                Grid_btn::add_button('oem_log', $params, true);

                                $grid = new Grid();
                                $grid->status_column_data = $status_column_data;
                                //$grid->filterable = false;
                                $grid->show_paging_bar = false;
                                $grid->grid_buttons = ['edit', 'delete', 'status' => ['status' => 'status'], 'view', 'duplicate', 'add_work_operation', 'workstation_log'];

                                $grid->init($paginate_OBJ, $query);

                                $grid->dt_column(['id' => ['title' => 'ID', 'width' => '20', 'align' => 'center', 'th_align' => 'center', 'hide' => true]]);
                                $grid->dt_column(['category_id' => ['hide' => true]]);
                                $grid->dt_column(['category' => [
                                    'wrap' => function($value, $field, $_row) use($_this) {
                                        if(user_do_action('view', 'workstation_categories')){
                                            return "<a target='_blank' href='".admin_url("workstation_categories/view/{$_row['category_id']}")."'>{$value}</a>";
                                        }
                                        return $value;
                                    }
                                ]]);
